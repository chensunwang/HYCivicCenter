...
NSString *const moreRouteCell = @"moreRouteCell";
@implementation MoreRouteViewController
...
- (void)loadDataWithPage:(NSInteger)page {
    ...
    [HttpRequest getPath:@"/phone/v2/bus/searchBus" params:@{@"keyword":self.searchTF.text.length >0?self.searchTF.text:self.keyword,@"pageNum":@(page),@"pageSize":@"20",@"type":@"line"} resultBlock:^(id  _Nullable responseObject, NSError * _Nullable error) {
        ...
        if ([responseObject[@"code"] intValue] == 200) {
            ...
            NSArray *datas = [HYSearchLineModel mj_objectArrayWithKeyValuesArray: responseObject[@"data"][@"line"][@"records"]];
...
- (void)configUI {
    ...
    [self.tableView registerClass:[SearchStationTableViewCell class] forCellReuseIdentifier:moreRouteCell];
...
//    searchTF.placeholder = @"查路线、站点";
    ...
    searchTF.attributedPlaceholder = [[NSAttributedString alloc]initWithString:@"查路线" attributes:attrs];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SearchStationTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:moreRouteCell];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    
    cell.lineModel = self.datasArr[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HYSearchLineModel *lineModel = self.datasArr[indexPath.row];
