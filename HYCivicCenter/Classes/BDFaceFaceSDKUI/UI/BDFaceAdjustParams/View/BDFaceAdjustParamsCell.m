...
static float const BDFaceAdjustParamsLeftButtonRightMargin = 0.0f;
...
- (void)configisChange:(NSNotification *)notification {
    ...
    if (item.contentType == BDFaceAdjustParamsContentTypeRecoverToNormal) {
        ...
        if ([BDFaceCalculateTool noNullDic:dic]) {
            ...
            if (number.boolValue) {
                self.textLabel.textColor = [UIColor colorAndAlphaWithHexString:BDFaceAdjustParamsRecoverUnactiveTextColorNew];
            } else {
                self.textLabel.textColor = [UIColor colorAndAlphaWithHexString:BDFaceAdjustParamsRecoverActiveTextColorNew];
            }
        }
    } else {
        self.textLabel.textColor = [UIColor blackColor];
...
- (void)resetPlusAndMinusButtons {
    ...
    if (currentValue.floatValue == minValue.floatValue) {
        ...
        [self.leftButton setImage:HyBundleImage(@"left_button_unable") forState:UIControlStateHighlighted];
    } else {
    ...
    if (currentValue.floatValue == maxValue.floatValue) {
        ...
        [self.rightButton setImage:HyBundleImage(@"right_button_unable") forState:UIControlStateHighlighted];
    } else {
