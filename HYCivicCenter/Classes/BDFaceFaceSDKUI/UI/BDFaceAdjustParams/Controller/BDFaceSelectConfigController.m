...
- (nullable UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    label2.textColor = [UIColor face_colorWithRGBHex:0x91979E];
...
- (void)updateCellContent:(UITableViewCell *)cell indexPath:(NSIndexPath *)indexPath {
    ...
    if (theCell) {
        theCell.adjustConfigAction = ^(BDFaceSelectType type) {
            [this toAdjustParamsPage:type];
        };
        
    }
}

- (void)toAdjustParamsPage:(BDFaceSelectType)type {
    ...
    [self.navigationController pushViewController:adjustController animated:YES];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[BDFaceSelectConfigCell class]]) {
        ...
        if (typeChoosed == BDFaceSelectTypeCustom) {
            [self toAdjustParamsPage:typeChoosed];
        }
    }
}
