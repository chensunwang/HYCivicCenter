...
@interface BDPopupController () <UIGestureRecognizerDelegate> {
    NSTimer *_timer;
...
@implementation BDPopupController
...
- (instancetype)initWithView:(UIView *)aView size:(CGSize)size {
    if (self = [super init]) {
        ...
        self.defaultDismissBlock = ^(BDPopupController * _Nonnull popupController) {
            [_self dismiss];
        };
    }
    return self;
...
static void *UIViewBDPopupControllersKey = &UIViewBDPopupControllersKey;

@implementation UIView (BDPopupController)

- (void)zh_presentPopupController:(BDPopupController *)popupController completion:(void (^)(void))completion {
    return [self zh_presentPopupController:popupController duration:0.25 completion:completion];
}

- (void)zh_presentPopupController:(BDPopupController *)popupController duration:(NSTimeInterval)duration completion:(void (^)(void))completion {
    return [self zh_presentPopupController:popupController duration:duration bounced:NO completion:completion];
}

- (void)zh_presentPopupController:(BDPopupController *)popupController duration:(NSTimeInterval)duration bounced:(BOOL)isBounced completion:(void (^)(void))completion {
...
- (void)zh_presentPopupController:(BDPopupController *)popupController duration:(NSTimeInterval)duration delay:(NSTimeInterval)delay options:(UIViewAnimationOptions)options bounced:(BOOL)isBounced completion:(void (^)(void))completion {
    NSMutableArray<BDPopupController *> *_popupControllers = objc_getAssociatedObject(self, UIViewBDPopupControllersKey);
    if (!_popupControllers) {
        _popupControllers = [[NSMutableArray alloc] init];
        objc_setAssociatedObject(self, UIViewBDPopupControllersKey, _popupControllers, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    
    popupController.proxyView = self;
    
    if (_popupControllers.count > 0) {
        [_popupControllers sortUsingComparator:^NSComparisonResult(BDPopupController *obj1, BDPopupController *obj2) {
        ...
        if (popupController.windowLevel >= _popupControllers.lastObject.windowLevel) {
            ...
            for (BDPopupController *element in _popupControllers) {
...
- (void)zh_dissmissPopupController:(BDPopupController *)popupController completion:(void (^)(void))completion {
    return [self zh_dissmissPopupController:popupController duration:0.25 completion:completion];
}

- (void)zh_dissmissPopupController:(BDPopupController *)popupController duration:(NSTimeInterval)duration completion:(void (^)(void))completion {
...
- (void)zh_dissmissPopupController:(BDPopupController *)popupController duration:(NSTimeInterval)duration delay:(NSTimeInterval)delay options:(UIViewAnimationOptions)options completion:(void (^)(void))completion {
    NSMutableArray<BDPopupController *> *_popupControllers = objc_getAssociatedObject(self, UIViewBDPopupControllersKey);
    if (_popupControllers.count > 0) {
        ...
        if (_popupControllers.count < 1) {
            objc_setAssociatedObject(self, UIViewBDPopupControllersKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        }
    }
}
...
@implementation BDPopupController (Convenient)
